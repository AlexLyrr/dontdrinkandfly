...
void setPacket(struct pcState* pcState, SRPacket* sPacket) {
	...
	if (DroneStatusMode == 7 && (pcState->aPressed || pcState->zPressed || pcState->jLift || pcState->n7Pressed)) {
		sPacket->payload[0] = 5;
		pcState->mode = 5;
	}
...
void logReceivePacket(SRPacket rPacket) {
	...
	switch (rPacket.payload[0]) {
		...
		case 12:
			...
			break;
		case 13:
			val3 = ((uint64_t)rPacket.payload[1] << 56) | ((uint64_t)rPacket.payload[2] << 48) | ((uint64_t)rPacket.payload[3] << 40) | ((uint64_t)rPacket.payload[4] << 32) | ((uint64_t)rPacket.payload[5] << 24) | ((uint64_t)rPacket.payload[6] << 16) | ((uint64_t)rPacket.payload[7] << 8) | ((uint64_t)rPacket.payload[8]);
