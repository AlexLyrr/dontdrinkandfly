...
function sendPacket(buf) {
	var packet = Buffer.alloc(buf.length + 5, 0);
	buf.copy(packet, 4);
	packet.writeUInt16BE(0x1337, 0);
	packet.writeUInt16BE(packetNumber % 65536, 2);
	let crc = 0x00;
	for (let i = 0; i < buf.length + 2; i += 1) {
		crc = crc8_table[crc ^ packet[i + 2]];
	}
...
port.on('data', function(data) {
	...
	let last = 0;
	for (let i = 0; i < appBuffer.length - 1; i += 1) {
		if (i + 15 > appBuffer.length) {
			break;
		}
		if (appBuffer[i] == 0x13 && appBuffer[i + 1] == 0x37) {
			onData(appBuffer.slice(i, i + 15));
			last = i + 15;
		}
	}
	appBuffer = appBuffer.slice(last, appBuffer.length);
	// if (data.length === 15) {
	// } else {
