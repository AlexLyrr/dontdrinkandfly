		...
		while((i + PACKET_LENGTH) <= rx_queue.count && i < MAX_PACKET_SCAN) {
			if (queuePeek(&rx_queue, 0) == PREAMPLE_B1 && queuePeek(&rx_queue, 1) == PREAMPLE_B2) {
				...
				for (uint8_t o = 2; o < (PACKET_LENGTH - 1); o += 1) {
					crc = crc8_table[crc ^ queuePeek(&rx_queue, o)];
				}
				if (crc == queuePeek(&rx_queue, PACKET_LENGTH - 1)) {
					NVIC_DisableIRQ(UART0_IRQn);
					for(uint8_t o = 0; o < PACKET_LENGTH; o += 1) {
						state.currentPacket[o] = dequeue(&rx_queue);
					}
...
void writePacket(uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3, uint8_t b4, uint8_t b5, uint8_t b6, uint8_t b7, uint8_t b8, uint8_t b9) {
	...
	uint8_t crc = 0x00;
	uint16_t packetNumber = state.packetNumber++;
